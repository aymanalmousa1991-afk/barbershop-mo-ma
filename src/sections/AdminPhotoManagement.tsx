...
export function AdminPhotoManagement() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/admin/photos`, {
        method: 'POST',
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await fetch(`${API_URL}/admin/photos/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      await fetchPhotos();
        ...
        <Card className="border-0 shadow-lg">
          <CardHeader className="bg-gradient-to-r from-[#6b0f1a] to-[#8b1523]">
            <CardTitle className="text-white flex items-center gap-2">
              <Upload className="h-5 w-5 text-[#d4af37]" />Foto Uploaden
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6 space-y-4">
            ...
            <div>
              ...
              <Input value={caption} onChange={(e) => setCaption(e.target.value)} placeholder="Bijv. 'Nieuw kapsel trend'" />
            </div>
            {selectedFile && (
              <div className="flex items-center gap-2 text-sm text-stone-600">
                <span>Geselecteerd: {selectedFile.name}</span>
                <button onClick={() => setSelectedFile(null)} className="text-red-500 hover:text-red-700">
                  <X className="h-4 w-4" />
                </button>
              </div>
            )}
            <Button onClick={handleUpload} disabled={!selectedFile || uploading} className="w-full bg-[#6b0f1a]">
              {uploading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Bezig met uploaden...</> : 'Uploaden'}
            </Button>
          </CardContent>
        </Card>
              ...
              <div className="grid grid-cols-2 gap-3 max-h-96 overflow-y-auto">
                {photos.map((photo) => (
                  <div key={photo.id} className="relative group aspect-square rounded-lg overflow-hidden shadow">
                    <img
                      src={`${API_URL}/uploads/photos/${photo.filename}`}
