...
<body>
  ...
  <div class="stats">
    ...
    <div class="stat-card"><div class="number">€\${r.totalRevenue.toFixed(2)}</div><div class="label">Totale Omzet</div></div>
  ...
  <table>
    ...
    \${rows}
    <tr><td><strong>TOTAAL</strong></td><td><strong>\${r.totalAppointments}</strong></td><td><strong>€\${r.totalRevenue.toFixed(2)}</strong></td></tr>
  </table>
...
c = c.replace(
  '<Button onClick={downloadHTML} className="bg-[#6b0f1a]"><Download className="h-4 w-4 mr-2" />Download als HTML</Button>',
...
console.log("✅ AdminReports.tsx updated");

// ===== AdminSettings.tsx =====
let s = fs.readFileSync("src/sections/AdminSettings.tsx", "utf8");
// Count "Opslaan" occurrences
const opslaanCount = (s.match(/Opslaan/g) || []).length;
console.log("Aantal 'Opslaan' buttons in AdminSettings:", opslaanCount);
